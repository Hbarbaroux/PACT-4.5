...
public class Note {
	...
	private static ArrayList<Complex> conjugate(ArrayList<Complex> echantillon) 
	{
		ArrayList<Complex> res = new ArrayList<Complex>();
		for (int k=0; k<echantillon.size(); k++) 
		{
			Complex a = echantillon.get(k);
			echantillon.remove(k);
			echantillon.add(k, a.conjugate());
		}
		return echantillon;
	}
	...
	private static ArrayList<Complex> fft(ArrayList<Float> signal) 	// fft adjusted to power of two (zero-padding)
	{
		...
		while ((int)Math.pow(2,k) < 16*signal.size()) //finds first power of 2 higher then 16 times the size of the array
		{
			k++;
		} 
